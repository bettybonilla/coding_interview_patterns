...
def zero_striping(matrix: list[list[int]]):
    ...
    # When a 0 is found at matrix[r][c], we place markers at:
    # matrix[0][c]  -> row r should be zeroed
    # matrix[r][0]  -> column c should be zeroed
    #
    # Example:
    #
    # Original matrix
    ...
    # If the first row originally contained a 0, zero the entire first row.
    if first_row_has_zero:
        for c in range(len(matrix[0])):
            matrix[0][c] = 0

    # If the first column originally contained a 0, zero the entire first column.
